...
MULTI_IMAGE_DIR = 'multi_label_images'
...
def get_image_labels_path(image_lists, label_name, index, image_labels_dir, category):
    ...
    full_path = os.path.join(image_labels_dir, MULTI_IMAGE_DIR, base_name)
    full_path += '.txt'
    return full_path
...
def get_ground_truth(labels_file, label):
...
def get_random_cached_bottlenecks(sess, image_lists, how_many, category,
                                  ...
                                  decoded_image_tensor, resized_input_tensor,
                                  bottleneck_tensor, architecture, label):
    ...
    if how_many >= 0:
        ...
        for unused_i in range(how_many):
            ...
            bottleneck = get_or_create_bottleneck(
                ...
                bottleneck_dir, jpeg_data_tensor, decoded_image_tensor,
                resized_input_tensor, bottleneck_tensor, architecture)
            labels_file = get_image_labels_path(image_lists, label_name, image_index, FLAGS.image_dir, category)
            ground_truth = get_ground_truth(labels_file, label)
...
def get_random_distorted_bottlenecks(
        ...
        distorted_image, resized_input_tensor, bottleneck_tensor, label):
        ...
        with tf.Session(graph=graph) as sess:
            ...
            (train_step, cross_entropy, bottleneck_input, ground_truth_input,
             final_tensor) = add_final_training_ops(
                2, FLAGS.final_tensor_name, bottleneck_tensor,
            ...
            train_writer = tf.summary.FileWriter(FLAGS.summaries_dir + '/' + label + '_train',
                                                 sess.graph)

            validation_writer = tf.summary.FileWriter(
                FLAGS.summaries_dir + '/' + label + '_validation')
                ...
                if do_distort_images:
                    (train_bottlenecks,
                     train_ground_truth) = get_random_distorted_bottlenecks(
                        ...
                        distorted_image_tensor, resized_image_tensor, bottleneck_tensor, label)
                else:
                    (train_bottlenecks,
            ...
            tf.logging.info('Final test accuracy for %s = %.1f%% (N=%d)' %
                            (label, test_accuracy * 100, len(test_bottlenecks)))
            ...
            # constants.
            save_graph_to_file(sess, graph, FLAGS.output_graph.format(label))
            ...
            del validation_ground_truth
            del validation_bottlenecks
            del train_accuracy
            del train_bottlenecks
            del train_ground_truth
            del train_step
            del train_summary
            del bottleneck_input
            del bottleneck_tensor
            del cross_entropy
            del ground_truth_input
            del test_accuracy
            del test_bottlenecks
